window.addEventListener('load', function () {
        ...
        if (window.innerWidth > 768) {
            ...
            if (allowScrolling) {
                ...
                if (detectMouseWheelDirection(e) == 'up' && currentSection != 0)
                    sections[--currentSection].scrollIntoView({ behavior: 'smooth', alignToTop: true })
                else if (detectMouseWheelDirection(e) == 'down' && currentSection != 4)
                    sections[++currentSection].scrollIntoView({ behavior: 'smooth', alignToTop: true })

                setTimeout(function () { allowScrolling = true }, 500)
            }
        }
    }, { passive: false })
    ...
    for (var m = 0; m < navItems.length; m++) {
        navItems[m].addEventListener('click', function (e) {
            ...
            if (allowScrolling && !isInViewport(document.querySelector('#' + this.getAttribute('href')))) {
                allowScrolling = false
    ...
    document.addEventListener('keydown', function (e) {
        if (e.key == 'ArrowUp' && allowScrolling) {
            if (currentSection == 0)
                return
        ...
        } else if (e.key == 'ArrowDown' && allowScrolling) {
            if (currentSection == 4)
                return
